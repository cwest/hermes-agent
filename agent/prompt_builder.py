...
KANBAN_GUIDANCE = (
    ...
    "(`{changed_files: [...], tests_run: N, decisions: [...]}`). Never put "
    "secrets / tokens / raw PII in either — run rows are durable forever. "
    "`kanban_complete` sets the card to `done` (≡ *merged*), so use it only for "
    "non-PR work finished outright.\n"
    "5a. **Code change that opens a PR (most coding tasks): open the "
    "marker-stamped draft PR, comment the URL, and END THE RUN — do NOT "
    "`kanban_complete`, do NOT self-block.** The PR body must carry the "
    "`<!-- card:$HERMES_KANBAN_TASK -->` marker; drop the PR URL + head SHA "
    "into a `kanban_comment`, then stop. The `github-prs` webhook + "
    "`stage-pr-review` does the `running → review` move and assigns the "
    "reviewer — you never move the card. `kanban_complete` here would mark an "
    "unmerged draft `done`. Reserve `kanban_block` only for a genuine "
    "needs-input fork (a real human decision).\n"
    ...
    "\"${HERMES_KANBAN_BRANCH:?}\"` from the main repo (the dispatcher always "
    "sets a descriptive kebab-case branch, e.g. `fix/<slug>` / `feat/<slug>`), "
    "then cd there. For a project-linked task the workspace is a fresh "
    "`<repo>/.worktrees/<descriptive-slug>` and `$HERMES_KANBAN_BRANCH` a "
    "deterministic `<project-slug>/<descriptive-slug>` — the main repo is two "
    "levels up, so run "
