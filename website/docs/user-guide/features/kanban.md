...
The dispatcher refuses to re-spawn a ready task when it hit a quota/auth/429 error on the previous run (`blocker_auth`), or completed a run successfully within the guard window (`recent_success`), or a recent task comment links to a GitHub PR at or after the latest explicit unblock (`active_pr`). This prevents repeat worker storms on the same bug or task while a human catches up, while still allowing an explicit unblock to wake the same task to address fresh PR review feedback when only older PR-handoff comments contain raw PR URLs. See the `respawn_guarded` row in the [event reference](#event-reference).
