...
| `authors` | No | List of account logins allowed to trigger this route (e.g. `["cwest"]`). **Security hard gate** — evaluated before the prompt is rendered or a worker is spawned, so a delivery from a non-allow-listed author is ignored at the HTTP layer and never starts a run on untrusted external input. If empty/omitted, all authors are accepted (no change to existing routes). The author is resolved per event type: `pull_request` / `pull_request_review` → `pull_request.user.login`; `push` → `sender.login` (then `pusher.name`); other events → `sender.login`. Unlike `actions`, this gate fails **closed**: if an allow-list is set but no author can be resolved from the payload, the delivery is ignored. |
    ...
    extra:
      ...
      routes:
        github-pr:
          ...
          authors: ["cwest"]  # security gate: only act on PRs from these logins
