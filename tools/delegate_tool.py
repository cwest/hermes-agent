...
def _strip_blocked_tools(toolsets: List[str]) -> List[str]:
    ...
    # ``code_execution`` is deliberately NOT blocked: upstream #69325 lets
    # subagents use execute_code. ``kanban`` is stripped so a delegated child
    # can never drive the board (its lifecycle verbs belong to the parent
    # worker alone — a child calling kanban_complete false-dones the card).
    _COMPOSITE_BLOCKED_TOOLSETS = frozenset({"delegation", "kanban"})
